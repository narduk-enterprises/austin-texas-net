...
// ─── User Operations ────────────────────────────────────────
export async function createUser(event: H3Event, email: string, password: string, name?: string): Promise<User> {
...
export async function getUserById(event: H3Event, id: string): Promise<User | undefined> {
  const db = useDatabase(event)
  return db.query.users.findFirst({
...
export async function getUserByAppleSub(event: H3Event, appleSub: string): Promise<User | undefined> {
  const db = useDatabase(event)
  return db.query.users.findFirst({
