...
<template>
  <UApp>
    <!-- Skip to main content for keyboard users (visible on focus) -->
    <ULink
      to="#main-content"
      class="skip-link"
    >
            ...
            <div v-for="col in footerColumns" :key="col.label">
              ...
              <ul class="space-y-3">
                <li v-for="link in col.children" :key="String(link.to)">
                  <NuxtLink
                    :to="link.to"
                    class="inline-block rounded px-1 -mx-1 py-0.5 text-sm text-primary dark:text-muted hover:text-primary dark:hover:text-white whitespace-normal wrap-break-word cursor-pointer transition-colors duration-200"
                  >
                    {{ link.label }}
                  </NuxtLink>
                </li>
