...
const dopplerConfigPath = [join(process.cwd(), 'doppler.yml'), join(process.cwd(), 'doppler.yaml')].find((p) => existsSync(p))
...
/** Read doppler.yml / doppler.yaml (project + config) if present. */
function readDopplerYaml(): { project: string; config: string } | null {
  if (!dopplerConfigPath) return null
  const raw = readFileSync(dopplerConfigPath, 'utf8')
