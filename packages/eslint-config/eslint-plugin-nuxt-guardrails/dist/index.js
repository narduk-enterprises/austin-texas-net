  ...
  create(context) {
    ...
    return parserServices.defineTemplateBodyVisitor(
      {},
      {
              ...
              for (const prop of obj.properties) {
                if (prop.type === "Property" && prop.key && (prop.key.type === "Identifier" && prop.key.name === "head" || prop.key.type === "Literal" && prop.key.value === "head")) {
                  context.report({
                    ...
                    data: { docUrl },
                    fix(fixer) {
                      if (prop.value && prop.value.type === "ObjectExpression" && prop.value.properties.length > 0) {
                        const firstProp = prop.value.properties[0];
                        if (firstProp.type === "Property" && firstProp.key && firstProp.key.type === "Identifier" && (firstProp.key.name === "title" || firstProp.key.name === "description") && firstProp.value && firstProp.value.type === "Literal") {
        ...
        'MethodDefinition[key.name="head"]'(node) {
          context.report({
            node,
  ...
  create(context) {
    ...
    return parserServices.defineTemplateBodyVisitor(
      {},
      {
        ...
        'MethodDefinition[key.name="fetch"]'(node) {
          context.report({
            node,
